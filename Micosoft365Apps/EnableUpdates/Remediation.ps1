...
Try {
    ...
    If(Test-path $64bitPath)
        {
            Remove-ItemProperty -Path $64bitPath -Name $Name -ErrorAction SilentlyContinue -Force
        }

    If(Test-Path $32bitPath)
        {
            Remove-ItemProperty -Path $32bitPath -Name $Name -ErrorAction SilentlyContinue -Force
        }
